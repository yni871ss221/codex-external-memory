...
- アプリケーションアイコン、クレジット画面、起動時スタジオロゴを追加し、スタジオロゴはアプリ起動時の1回だけ表示するようにした。
- タイトル、強化、武器図鑑、所持レリック、ゲーム終了画面へ背景画像を追加し、クリア時は夜明け、敗北時は暗い背景へ分岐した。
- 自陣境界へ青い境界線を追加した。敵陣の赤境界は不採用として差し戻した。
- キーマウ/パッド入力モード、共通フォーカス、方向移動、Dropdown、Slider、スクロール追従、レベルアップ決定を共通基盤へ整理した。
- レベルアップ画面へ各ラン3回のスキップ/リロールを追加し、残数表示、0回時無効化、開幕連続レベルアップへの接続を実装した。
- サブエージェント構成を追加したが、軽微修正で約1時間かかった結果を受け、ユーザーが明示指定した作業でだけ使用する運用へ変更した。
- C#コンパイル成功。スキップ/リロールの最終Play Mode操作確認は次チャットへ引き継ぐ。
...
- オプション画面をタイトル/ゲーム内で整理し、一般/サウンド/グラフィック/コントロールの縦スクロール、表示モード/ウィンドウサイズ、音量%表示、設定初期化、タイトル専用データ初期化ダイアログを追加した。
- キーボード/マウスとコントローラーのキーコンフィグを実装し、入力待ち方式、×決定/〇キャンセル、ゲーム中キャンセルで一時停止、音量選択中左右1%変更を調整した。
- UIフォーカス移動を共通化し、直感的な上下左右移動、画面外フォーカス抑制、パッド操作時のみスクロール/パン追従、マウス操作時のスクロール優先を整理した。
- ロビー、オプション、一時停止、レベルアップ、宝箱獲得、データ初期化/諦めるダイアログ、強化画面などでフォーカスとキャンセル動作を調整した。
- スキルツリーへ「開幕レベルアップ」を追加し、Generatedアイコンを割り当て、ゲーム開始時に獲得回数分のレベルアップパネルを表示するようにした。
- 難易度解放仕様を、初回クリアで難易度1〜2解放、難易度2〜4ボス撃破で次難易度解放へ変更した。難易度2以降はボス撃破後もゲーム継続する。
- トークン獲得ログとアプリログをBuild配下 `logs` へ出力し、到達ステージ、ボスクリア、Material/Memory診断を記録するようにした。
- 中心塔破壊クラッシュ調査からMaterial大量増殖を特定し、バリスタ/壁の完成色変更を `MaterialPropertyBlock` と `sharedMaterials` へ移行した。
- アウトラインを黒塗りつぶし方式へ変更し、ヒット時白ハイライトと遮蔽シルエットにも同じ領域を使うよう整理した。ヒット時にYSort優先度が一瞬変わる問題も修正した。
- ファイアボール軌道の床塗りを復旧し、スラッシュ攻撃範囲とレベルアップ伸び幅を調整した。
- オークキング衝撃波は倍率1の24ダメージ、ゴブリンロード闇玉は多段Hit対策で1Hit 12ダメージ相当に調整した。
