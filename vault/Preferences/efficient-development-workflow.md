...
## 2026-07-13 Unity検証の厳格な上限

- ユーザーが「時間をかけてもよい」「徹底検証してよい」と明示しない限り、作業規模を問わずCompile最大2回、Play Mode開始最大2回を標準上限とする。
- 失敗、タイムアウト、再試行、確認目的の再実行も回数へ含める。
- 3回目が必要になった時点でコード変更を止め、Editor設定、Active Scene、参照、ライフサイクル、Console、テスト経路を読み取り確認する。追加許可なしに3回目を実行しない。
- 想定外の結果後は、仮説修正を反復せず原因調査を先に行う。
- 同種コマンド失敗や引数/エスケープミスが2回続いたら、手打ちをやめて `Tools/TokenUsage` Wrapper、一時Editor Runner、Reporter、Validatorへ部品化する。
- 権限やUnity接続タイムアウトは実装不良と分離し、Unity状態を確認して同じ操作を最大1回だけ再試行する。
- Fast Pathは対象特定、一括反映、Compile 1回、代表表示1件、Console Error 1回を目安とし、確認済み経路を繰り返さない。